...
export function PejabatPage() {
  ...
  const [deletePejabatItem, setDeletePejabatItem] = useState<Pejabat | null>(null);
  ...
  const fetchPejabat = async () => {
    try {
      setLoading(true);
      const json = await api.get<any>("/pejabat");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/pejabat/${editingId}`, payload);
      } else {
        await api.post("/pejabat", payload);
      }
      toast("success", `Data pejabat berhasil ${editingId ? "diperbarui" : "ditambahkan"}`);
      setShowForm(false);
      fetchPejabat();
    } catch (e: any) {
      toast("error", e.message || "Gagal menyimpan data pejabat");
    }
  };

  const confirmDelete = async () => {
    if (!deletePejabatItem) return;
    try {
      setIsDeleting(true);
      await api.delete(`/pejabat/${deletePejabatItem.id}`);
      toast("success", "Data pejabat berhasil dihapus");
      setDeletePejabatItem(null);
      fetchPejabat();
    } catch (e: any) {
      toast("error", e.message || "Gagal menghapus data pejabat");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
              ...
              },
              {
                ...
                render: (row) => (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setDeletePejabatItem(row)}
      ...
      <ConfirmDialog
        open={!!deletePejabatItem}
        onClose={() => setDeletePejabatItem(null)}
        onConfirm={confirmDelete}
        title="Hapus Data Pejabat"
        description={`Apakah Anda yakin ingin menghapus data pejabat "${deletePejabatItem?.nama}"? Tindakan ini bersifat permanen dan tidak dapat dibatalkan.`}
        confirmText="Hapus Pejabat"
        cancelText="Batal"
        variant="danger"
